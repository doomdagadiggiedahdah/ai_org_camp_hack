...
def analyze_document_with_rules(document_text, rules):
    ...
    prompt += f"Rules:\n{rules}\n\nDocument:\n"
    prompt += ("Please return the result in a csv file with the format of: "
               "idx, purpose of the purchase, amount, confidence interval that this satisfies rules, date")
...
def run_rules_on_document(document_path, file_type):
    ...
    for row in rows:
        if len(row) != len(header):
            print("Row length mismatch:", row)
            print(f"Expected {len(header)} columns but got {len(row)} columns.")
            return
